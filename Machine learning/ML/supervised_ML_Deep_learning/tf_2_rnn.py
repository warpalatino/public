...
# [3] split dataset 
# ------
# -- monthly data, 325 datapoints; keep 4% for testing (i.e. data from last year, 12 values)
limit = round(len(df)*0.96)
# print(limit)
train = df.iloc[:limit]
test = df.iloc[limit:]
# print(train)
...
# ------
n_input = 12     # how many values the model look at to predict the next one; seasonal dataset may require the entire seasonal range for good predictions
n_features = 1  # always 1 for time series, as there is always only one timestamp per datapoint
batch_size = 1  # batch to use at each round
...
model.add(layers.LSTM(150))     # 150 = neurons here
...
# ------
model_run = model.fit_generator(generator, epochs=18)
