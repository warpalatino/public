...
# ------------
# for details on the model: https://scikit-learn.org/stable/auto_examples/decomposition/plot_pca_iris.html 
pca = PCA(n_components = 2) # we can experiment with how many extracted components the model will be able to classify (if results are poor with 2, we grow to 3 or above...)
X_train = pca.fit_transform(X_train)    # we fit and transform
X_test = pca.transform(X_test)  # we only transform as we do not want to fit our data here as we use it only to verify information
